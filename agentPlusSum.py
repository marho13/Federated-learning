...
class runner:
    ...
    def initialiseRun(self, state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip, action_std, env_name, num_agents, netsize):
        ...
        self.episode = 0
        self.time_step = 0
        weight = self.ps.get_weights()
        [w.setWeights.remote(weight) for w in self.worker]
    ...
    def runInitializations(self, state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip, action_std, env_name, num_agents, netsize, netstring):
        for a in range(self.num_runs):
            self.wandy = wandb.init(project=env_name + "-{}".format(netstring), group="ActorSum",
                                    config={
                                        "batch_size": 16,
                                        "learning_rate_critic": lr_critic,
                                        "learning_rate_actor": lr_actor,
    ...
    def workerRun(self):
        ...
        for _ in range(self.k_epoch):
            ...
            gradients = self.ps.getActorGradSum(grad)
            [self.worker[a].applyGrads.remote(gradients[a]) for a in range(len(gradients))]
