...
class DataWorker(object):
    ...
    def compute_gradients(self, weights):
        ...
        # print("Done with gym")
        self.DQN.update(self.memory, 64)
        return [self.DQN.policy_net.get_gradients(), rew]

    def common_sum_self(self, gradients):
        summed = [np.stack(grad).sum(axis=0) for grad in [gradients, self.DQN.policy_net.get_gradients()]]
        self.optimizer.zero_grad()
        self.DQN.policy_net.set_gradients(summed)
        self.optimizer.step()
...
for i in range(iterations):
    ...
    summedGrad = ps.apply_gradients.remote(*gradients) #Calculate the summed grad of the workers

    print(summedGrad.shape)

    [worker.common_sum_self.remote(summedGrad) for worker in workers] #Getting the summed gradients, and adding the specific workers gradient
