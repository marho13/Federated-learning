...
class ParameterServer(object):
    ...
    def aboveAvg(self, grads, rewards, avgRew):
        indexes = self.aboveAvgIndexes(rewards, avgRew)
        output = []
        for i in indexes:
            output.append(copy(grads[i]))
        self.calcAvgGrad(*output)
        return self.get_weights()

    def aboveAvgIndexes(self, rewards, avgRew):
        output = []
...
wandy = wandb.init(project="Federated-learning-PPO",
           config={
               ...
               "learning_rate": lr,
               "dataset": envName,
               "model": "single actor grad above average",
           })
...
for run in range(10):
    ...
    for i in range(iterations):
        ...
        current_weights = ps.aboveAvg(grads, reward, avgRew)
