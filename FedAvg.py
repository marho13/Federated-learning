...
class runner:
    def __init__(self, env_name, max_ep_len, has_continuous_action_space, k_epochs, eps_clip, gamma, lr_actor,
                 lr_critic,
                 action_std, action_std_decay_rate, min_action_std, action_std_decay_freq, log_f_name, netsize,
                 netstring):
        ...
        self.ppo_agent = PPO(state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip, self.has_continuous_action_space, action_std)
    ...
    def initialiseRun(self, state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip, action_std, env_name, num_agents, netsize):
        self.worker = [DataWorker.remote(state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip,
                                         self.has_continuous_action_space, action_std, env_name, netsize) for _ in
                       range(
    ...
    def runInitializations(self, state_dim, action_dim, lr_actor, lr_critic, gamma, k_epochs, eps_clip, action_std, env_name, num_agents, netsize, netstring):
        for a in range(self.num_runs):
            self.wandy = wandb.init(project=env_name + "-{}".format(netstring), group="FedAvg",
                                    config={
                                        "batch_size": 16,
            ...
            self.workerMax(maxEp=200, maxTi=int(1000))
            self.wandy.finish()
        self.wandy.finish()

    def workerRun(self):
        current_weights = self.ps.get_weights()
        [w.setWeights.remote(current_weights) for w in self.worker]
        r = ray.get([w.gatherReplay.remote() for w in self.worker])#ray.get(self.worker.gatherReplay())#
        rew = [a.item() for a in r]
        loss = ray.get([w.update.remote() for w in self.worker])#ray.get(self.worker.getLoss.remote(current_weights))#
        weights = ray.get([w.getWeights.remote() for w in self.worker])
        self.ps.avgWeights(weights)
...
# r.maxTime(maxTimesteps=int(3e6))
# r.workerMax(maxTi=int(3e6))
